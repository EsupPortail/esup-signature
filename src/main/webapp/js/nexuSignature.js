...
function getDataToSign(certificateData) {
    if(certificateData.response == null) {
        ...
        callUrl("https://esup-signature.univ-ville.fr/user/nexu-sign/get-data-to-sign", "POST",  JSON.stringify(toSend), sign, error);
    }
}
...
function signDocument(signatureData) {
    ...
    callUrl("https://esup-signature.univ-ville.fr/user/nexu-sign/sign-document", "POST", JSON.stringify(toSend), downloadSignedDocument, error);
}
