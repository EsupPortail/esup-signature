...
function getDataToSign(certificateData) {
    if(certificateData.response == null) {
        ...
        callUrl("https://esup-singature.univ-ville.fr/sign-a-pdf/get-data-to-sign", "POST",  JSON.stringify(toSend), sign, error);
    }
}
...
function signDocument(signatureData) {
    ...
    callUrl("https://esup-singature.univ-ville.fr/sign-a-pdf/sign-document", "POST", JSON.stringify(toSend), downloadSignedDocument, error);
}

function downloadSignedDocument(signDocumentResponse) {
    var url = signDocumentResponse.urlToDownload;
    url = "https://esup-singature.univ-ville.fr/sign-a-pdf/download";
