        ...
        $('.linkAnnotation').each(function (){
            ...
            $(this).droppable({
                ...
                drop: function( event, ui ) {
                    if($(event.originalEvent.target).attr("id") != null && ($(event.originalEvent.target).attr("id").includes("cross_") || $(event.originalEvent.target).attr("id").includes("border_"))) {
                        $("#border_" + $(event.originalEvent.target).attr("id").split("_")[1]).addClass("cross-error");
                    }
                },
                ...
                out: function( event, ui ) {
                    if($(event.originalEvent.target).attr("id") != null && ($(event.originalEvent.target).attr("id").includes("cross_") || $(event.originalEvent.target).attr("id").includes("border_"))) {
                        $("#border_" + $(event.originalEvent.target).attr("id").split("_")[1]).removeClass("cross-error");
                    }
                }
            });
        });
