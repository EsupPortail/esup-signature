...
@RequestMapping("/user/")
@Controller
@RooWebScaffold(path = "user", formBackingObject = User.class)
@Transactional
@Scope(value="session")
public class UserUIControler {
    ...
    public String index(Model uiModel) throws Exception {
		...
		if(User.countFindUsersByEppnEquals(eppn) > 0) {
			return "redirect:/user/viewsettings";
		} else {
			return "redirect:/user/updatesettings";		}
    }
	
    @RequestMapping(value = "/viewsettings", produces = "text/html")
    ...
    @RequestMapping(value = "/updatesettings", produces = "text/html")
