...
//				signedInputStream = pdfService.flattenOcg(signedInputStream);
	...
	public void setPaperlessSourceDocumentId(java.util.List<Long> signRequestIds, Long paperlessSourceDocumentId) {
		for (Long srId : signRequestIds) {
			SignRequest sr = getById(srId);
			if (sr != null) {
				sr.setPaperlessSourceDocumentId(paperlessSourceDocumentId);
			}
		}
	}

	@Transactional
	public void cleanAndLink(Long id, String externalUrl, String authUserEppn) {
		SignRequest signRequest = getById(id);
		if (signRequest != null) {
			signRequest.setExportedDocumentURI(externalUrl);
			signRequest.getLinks().add(externalUrl);
			clearAllDocuments(signRequest);
			signRequest.setStatus(SignRequestStatus.exported);
			signRequest.getParentSignBook().setStatus(SignRequestStatus.exported);
			updateStatus(signRequest.getId(), SignRequestStatus.exported, "Nettoyage et liaison externe Paperless", null, "SUCCESS", null, null, null, null, authUserEppn, authUserEppn);
		}
	}

	@Transactional
	public void cleanAndLinkWithPaperlessId(Long id, String externalUrl, Long paperlessSignedDocumentId, String authUserEppn) {
		SignRequest signRequest = getById(id);
		if (signRequest != null) {
			signRequest.setPaperlessSignedDocumentId(paperlessSignedDocumentId);
			cleanAndLink(id, externalUrl, authUserEppn);
		}
	}
    ...
    public FsFile getLastSignedFsFile(SignRequest signRequest) throws EsupSignatureFsException {
      signRequest = getById(signRequest.getId());
      if (!signRequest.getParentSignBook().getArchiveStatus().equals(ArchiveStatus.none)) {
        if (signRequest.getExportedDocumentURI() != null && !signRequest.getExportedDocumentURI().startsWith("mail")) {
          String uri = signRequest.getExportedDocumentURI();
          if (uri.startsWith("http://") || uri.startsWith("https://")) {
            try {
              ...
              String paperlessToken = userService.getSystemUiParam(UiParams.paperlessToken);
              if (StringUtils.hasText(paperlessToken)) {
                conn.setRequestProperty("Authorization", "Token " + paperlessToken);
              }
              InputStream inputStream = conn.getInputStream();
              String contentType = conn.getContentType();
              if (contentType == null) contentType = "application/pdf";
              String disposition = conn.getHeaderField("Content-Disposition");
              String fileName = signRequest.getTitle() + ".pdf";
              if (disposition != null && disposition.contains("filename=")) {
                fileName = disposition.replaceAll(".*filename=\"?([^\";\n]+)\"?.*", "$1").trim();
              }
              return new FsFile(inputStream, fileName, contentType);
            } catch (Exception e) {
              throw new EsupSignatureFsException("Erreur lors du téléchargement depuis l'URI : " + uri, e);
            }
          }
          FsAccessService fsAccessService = fsAccessFactoryService.getFsAccessService(uri);
          return fsAccessService.getFileFromURI(uri);
        }
      }
  ...
  @Transactional
	public Long delete(Long signRequestId, String userEppn) {
		...
		if(signRequest.getDeleted() || signRequest.getStatus().equals(SignRequestStatus.draft)) {
			...
			if (signRequest.getPaperlessSignedDocumentId() != null) {
				logger.info("suppression ignoree : demande " + signRequestId + " archivee dans Paperless (id=" + signRequest.getPaperlessSignedDocumentId() + ")");
  ...
  @Transactional
	...
	Long deleteDefinitive(Long signRequestId, String userEppn, boolean deleteEmptySignBook) {
		...
		SignBook signBook = signRequest.getParentSignBook();
		if (signRequest.getPaperlessSignedDocumentId() != null) {
			logger.info("suppression definitive annulee : demande " + signRequestId + " archivee dans Paperless (id=" + signRequest.getPaperlessSignedDocumentId() + ")");
			signRequest.setDeleted(false);
			signBook.setDeleted(false);
			return signBook.getId();
		}
...
//		}
		...
		for (Target target : signRequest.getParentSignBook().getLiveWorkflow().getTargets().stream().filter(t -> t != null && fsAccessFactoryService.getPathIOType(t.getTargetUri()).equals(DocumentIOType.rest)).toList()) {
			try {
				targetService.sendRest(target.getTargetUri(), signRequest.getId().toString(), "cleaned", signRequest.getParentSignBook().getLiveWorkflow().getCurrentStepNumber().toString(), userEppn, signRequest.getPaperlessSignedDocumentId() != null ? signRequest.getPaperlessSignedDocumentId().toString() : "");
			} catch (Exception e) {
				logger.error("error on sending deleted on rest " + target.getTargetUri());
			}
		}
    ...
    /**
     * Version optimisée de isDeletetable pour usage en boucle :
     * accepte un User et isAdmin déjà résolus par l'appelant afin d'éviter
     * N appels à userService.getByEppn() (fix problème N+1 / 504).
     *
     * @param signRequest la demande de signature à évaluer
     * @param user        l'utilisateur déjà chargé (pré-calculé hors boucle)
     * @param isAdmin     true si l'utilisateur a ROLE_ADMIN (pré-calculé hors boucle)
     * @return true si la demande est supprimable
     */
    public boolean isDeletetableFast(SignRequest signRequest, User user, boolean isAdmin) {
		if (isAdmin) return true;
		Workflow workflow = signRequest.getParentSignBook().getLiveWorkflow().getWorkflow();
		return workflow == null
				|| workflow.getDisableDeleteByCreator() == null
	...
	public boolean isSigned(SignRequest signRequest, Reports reports) {
		try {
			if(reports == null) {
				reports = signService.validate(signRequest.getId());
			}
	...
	public byte[] getZipWithDocAndReport(SignRequest signRequest, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Exception {
		...
		} else if (!signRequest.getArchiveStatus().equals(ArchiveStatus.none)) {
			FsFile fsFile = getLastSignedFsFile(signRequest);
			name = fsFile.getName();
			inputStream = fsFile.getInputStream();
		} else {
	...
	public List<SignRequest> getRecipientNotPresentSignRequests(String eppn) {
		...
		for(SignRequest signRequest : signRequests) {
			...
			for(Recipient recipient : recipients) {
				...
				if(user.getUserType().equals(UserType.azuread)
						|| user.getUserType().equals(UserType.external)
						|| user.getUserType().equals(UserType.shib)
						|| userService.findPersonLdapLightByUser(user) != null) {
	...
	public List<Long> getViewedSignRequestIds(List<Long> ids, String eppn) {
		if (ids.isEmpty()) return List.of();
		return signRequestRepository.findViewedSignRequestIds(ids, eppn);
	}

	public List<Long> getSignRequestIdsWithAttachments(List<Long> ids) {
		if (ids.isEmpty()) return List.of();
		return signRequestRepository.findSignRequestIdsWithAttachments(ids);
	}
