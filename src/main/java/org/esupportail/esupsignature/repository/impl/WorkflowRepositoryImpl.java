...
public class WorkflowRepositoryImpl implements WorkflowRepositoryCustom {
	...
	@Override
	public List<Workflow> findAuthorizedWorkflowByRoles(List<String> roles) {
		...
		if(roles.size() >0 ) {
			...
			query.where(criteriaBuilder.and(criteriaBuilder.or(publicUsageExpression.in(true), roleExpression.in(roles))));
		} else {
