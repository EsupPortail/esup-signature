...
public interface SignRequestRepository extends CrudRepository<SignRequest, Long>, PagingAndSortingRepository<SignRequest, Long> {
    ...
    @Query("select distinct s from SignRequest s join s.parentSignBook.liveWorkflow.currentStep.recipients r where s.status = 'pending' and :user not member of s.parentSignBook.hidedBy and r.user = :user")
    List<SignRequest> findByRecipientUserToSign(User user);
    ...
    @Query("""
            select count(distinct s.parentSignBook) from SignRequest s join s.parentSignBook.liveWorkflow.currentStep.recipients r where s.status = :status and (s.deleted is null or s.deleted != true) and s.parentSignBook.createBy.eppn = :createByEppn
            """)
