...
public interface SignRequestRepository extends CrudRepository<SignRequest, Long>, PagingAndSortingRepository<SignRequest, Long>, SignRequestRepositoryCustom {
    @Query("select count(distinct s) from SignRequest s where s.status = 'pending' and key(s.recipientHasSigned).user.eppn = :recipientUserEppn and key(s.recipientHasSigned).signed = false and key(s.recipientHasSigned) member of s.parentSignBook.liveWorkflow.currentStep.recipients")
    ...
    @Query("select distinct s from SignRequest s where s.status = 'pending' and key(s.recipientHasSigned).user.eppn = :recipientUserEppn and key(s.recipientHasSigned).signed = false and key(s.recipientHasSigned) member of s.parentSignBook.liveWorkflow.currentStep.recipients")
